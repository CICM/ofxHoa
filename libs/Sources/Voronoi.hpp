...
#define HOA_EPSILON 1e-15;
    ...
    public:
        ...
        Point()  : i(0), x(0), y(0), z(0), arc(NULL)
        {
            ;
        }
        
        Point(const T _x, const T _y, const T _z)  : i(0), x(_x), y(_y), z(_z), arc(NULL)
        {
            ;
        }
        
        static T dot(Point const&a, Point const&b) 
        {
            return a.x * b.x + a.y * b.y + a.z * b.z;
        }
        
        static Point subtract(Point const& a, Point const& b) 
        {
            return Point(a.x - b.x, a.y - b.y, a.z - b.z);
        }
        
        static Point cross(Point const& a, Point const& b) 
        {
            return Point(a.y * b.z - a.z * b.y, a.z * b.x - a.x * b.z, a.x * b.y - a.y * b.x);
        }
        
        static T norm2(Point const& d) 
        {
            return sqrt(dot(d, d));
        }
        
        static T norm(Point const& d) 
        {
            return sqrt(d.norm2());
        }
        
        static Point normalise(Point const& d)  
        {
            T m = 1 / d.norm();
    ...
    public:
        ...
        Edge(Triangle<T>* _t, const Point<T>& _p)  :
        t(_t),
        p(_p)
        {
    ...
    public:
        ...
        Arc(Triangle<T>* _t, const Point<T>& _p, const ulong _index)  :
        t(_t),
        p(_p),
        index(_index),
        prev(NULL),
    ...
    public:
        ...
        Triangle(const Point<T> _p1, const Point<T> _p2, const Point<T> _p3, const ulong _index = 0)  :
        index(_index),
        marked(false)
        {
            a = Edge<T>(this, _p1);
